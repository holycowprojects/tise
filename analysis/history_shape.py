...
Run, to **regenerate** an existing report from the corpus it was built on:
    uv run python analysis/history_shape.py --history data/history-chrome.copy

Run, to build a corpus for the **first** time, which opens the live browser profile:
    uv run python analysis/history_shape.py --browser Chrome

Without `--history` this script copies the live profile afresh, so the report describes a
browser that has moved on rather than the corpus every other benchmark used. D97 ran the
first form without `--history` and regenerated the page against three extra days.
...
from tise_research.reports import partly_superseded_banner

#: Anchors every default path. See `--data-dir` for what a cwd-relative default cost.
...
{partly_superseded_banner(
    "return_24h",
    stands=(
        "Visits per day, the gap distribution, session boundaries and domain "
        "concentration all stand"
    ),
    retired="The `return_24h` label-volume tables and the T1 gate",
)}
Generated by `analysis/history_shape.py` on {generated_at:%Y-%m-%d %H:%M %Z}.
**Do not edit by hand.** Regenerate with:

```
uv run python analysis/history_shape.py --history data/{copy_path.name} --out docs/benchmarks/
```
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        # Anchored to the repository, not the working directory. Run from `research/`, a
        # cwd-relative default writes a *second* copy of the live browser history to
        # `research/data/` — a directory no `.gitignore` rule covered, since the `/data/`
        # rule is anchored on purpose to spare the `tise_research/data/` source package.
        # That happened in D97. Both ends are fixed: the path here, and path-independent
        # `**/*.copy` rules in `.gitignore`.
        help="Local scratch directory, under the repository root. Gitignored.",
    )
    parser.add_argument(
        "--browser",
        default="Chrome",
