...
def _fold_table(evidence: CorpusEvidence) -> str:
    rows = [
        f"| Fold | Train | Test | `{MODEL_NAME}` | `{BAR_MODEL}` | Winner |",
        "|---:|---:|---:|---:|---:|---|",
    ]
...
def write_model_report(
    ...
    path.write_text(
        f"""# Model — `return_24h`, logistic regression over `{DEFAULT_FEATURE_SET}`
...
Logistic regression over the {len(FEATURE_NAMES)} features of `{DEFAULT_FEATURE_SET}`,
imputed and standardised by `models/prep.py` into {len(design_columns())} columns, optimised by
