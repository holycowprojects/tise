...
**`visit_engaged` is withheld, not pending** (D124). It cleared a pre-registered bar (D97)
and replicated on 1,326 people (D100), and it will not switch on: D99's data gate wants
1,000 watched visits and this profile's count **saturates near 656**, because raw events
expire at 30 days so the count is a window and not a running total (D123). Waiting does not
close it — the gap needs ~33 dwelled visits a day against ~22 observed.

**So there is no "wait for the gate" step any more.** Tise ships what it has measured.

### What is actually left

1. **T18** — packaging, listing copy, permission justifications. Needs Akash's three items
2. **README** — every number must trace to a committed script (Checkpoint E)
3. **Repo public** — before listing, never after (D122)
4. **Submit**

Nothing is blocked by anything except Akash's three Web Store items.

### Checkpoint E — five criteria, not twelve

**The "twelve criteria, only #7 outstanding" line was false and stood from the scaffold
commit until D124.** `SPEC.md` has listed five since `1e0c26d`; there was never a #7. It is
spelled out in [`plan.md`](plan.md) now rather than counted. Standing: **3, 4, 5 met**
(criterion 3 with a qualification that travels with it — read D124), **1 and 2** are
visibility and T18.

### Do not build these

- **T-G4's training path.** The gate is not met and is not going to be. Building a path
  nothing can exercise, while the records call it ready, is D101 exactly
- **T10b** — re-registered in **D121** as a prompting problem, not a clustering one. Bar,
  ranking rule and pessimistic counterfactual all written down, **nothing implemented on
  purpose**
- **A lower gate.** D123's third option (shipping may deserve a different bar from
  replication) is available, untaken, and carries its timing problem: it was noticed after
  the gate failed, by the person it excluded. It needs its argument written before any
...
- **CI is Ubuntu-only and it earns that.** Its first run found two Linux-only bugs, one in
  the parity suite (D120)
- **Dependabot ignores TypeScript majors** until `typescript-eslint` accepts one. The
  dev-tooling group merged 2026-09-21 (#6): eslint 9→10, vite 5→8, vitest 2→5, no code change
- Icons are **placeholders** (D117), generated by a committed script
- **The planning documents are not tested and have drifted before.** D124 found a false
  claim in this file that survived from the scaffold commit. Check them against `git log`,
  not memory

### Owed by Akash, and nothing else is

1. **Three Web Store items** — publish the privacy-policy section and send the URL; confirm
...
2. **Install on a fresh Chrome profile** — T15's own verification. Use a *second* profile:
   removing the extension deletes its IndexedDB
3. **The GESIS permission reply** into `docs/gesis-permission-request.md`. Blocks nothing

A fresh export is **no longer owed** — nothing is gated on one. It is still the way to check
collection is alive: `uv run python analysis/engagement_gate.py`.
...
## ⚠ Read this first — two targets retired, the third withheld

**The current state is the block at the very top of this file** (D124, 2026-09-21).
Everything from here down is history, kept because it is the evidence trail — including the
Phase 7 "START HERE" below, which was current until D124 and is not any more.
...
- **`visit_engaged` is adopted (D97)** — T-A cleared the bar D94 wrote down before the
  measurement, on both corpora with dwell.
...
- **`visit_engaged` REPLICATES on 1,326 other people (D100)** — +0.0091 [+0.0086, +0.0097],
  ...
  person.
- **`visit_engaged` is WITHHELD (D124)** — and this supersedes "not shipped yet" wherever
  the lines above imply a wait. D99's data gate is not met on this profile and does not
  close: the visit count is a 30-day window that saturates near 656 against a bar of 1,000
  (D123). **T-G4's training path is not to be built.** The extension still trains
  `return_24h`, still shows nobody a prediction, and the descriptive surfaces are the
  product.
